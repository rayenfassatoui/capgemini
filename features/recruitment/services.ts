...
export async function getStatisticsChatContext(userId: string): Promise<string> {
  const [cvs, allJobs, allCandidatesRaw, allInterviewsRaw, allScreeningsRaw] =
    await Promise.all([
      db
        ...
        .from(cvPool)
        .where(eq(cvPool.uploadedBy, userId)),
      db.select().from(jobs).where(eq(jobs.createdBy, userId)),
      db
        .select()
        .from(candidates)
        .orderBy(desc(candidates.createdAt)),
  ...
  sections.push(
    `## Candidate Pipeline (${allCandidatesRaw.length} total)\n- ${
  ...
  const recentCandidates = allCandidatesRaw.slice(0, 20);
...
export async function askAiAboutStatistics(
  ...
  // Gather aggregated data (no PII sent to LLM)
