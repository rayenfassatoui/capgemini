...
\begin{table}[H]
  \centering
  \caption{\label{tab:cv-jobs-backlog}Backlog fonctionnel du module vivier de CV et offres}
...
Même si le c\oe{}ur du pipeline candidat est traité dans le chapitre suivant, ce module prépare déjà le lien métier principal : le passage de \textit{CV dans le vivier} à \textit{candidat dans une offre}. Cette transition transforme un actif documentaire en objet de pipeline.
...
Cette interface montre ainsi que le vivier n'est pas une simple liste de fichiers. C'est une base de profils analysés, consultables et réexploitables, avec une lecture à la fois opérationnelle, analytique et déjà branchée sur l'assistance agentique. Les figures \ref{fig:cv-pool-page} et \ref{fig:cv-pool-details} illustrent respectivement la gestion du vivier et la consultation détaillée d'un profil.

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/cvpool-ta.png}
  \caption{\label{fig:cv-pool-page}Page de gestion du vivier de CV.}
\end{figure}
