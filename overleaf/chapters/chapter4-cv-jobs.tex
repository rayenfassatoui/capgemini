...
Le sprint met donc en place une page de gestion du catalogue, l'import de CV avec extraction automatique, l'indexation sémantique, la détection des doublons, la recherche et l'export du catalogue, la création et la gestion des offres ainsi que la préparation du matching et du futur pipeline candidat.
...
Le tableau \ref{tab:cv-jobs-backlog} présente les user stories qui concrétisent ce périmètre.

\subsection{Backlog du sprint}
\begin{table}[H]
  \centering
  \caption{\label{tab:cv-jobs-backlog}Backlog fonctionnel du module catalogue de CV et offres}
  \begin{tabularx}{\textwidth}{>{\bfseries}p{1.3cm}p{3cm}X>{\raggedright\arraybackslash}p{2cm}}
    \toprule
    ID & Thème & User Story & Statut \\
    \midrule
    7 & Catalogue de CV & En tant que TA, je peux importer et gérer des CV dans un catalogue centralisé. & Réalisé \\
    ...
    10 & Détection de doublons & En tant que TA, je peux identifier des doublons potentiels dans le catalogue. & Réalisé \\
    11 & Export documentaire & En tant que TA, je peux exporter un ou plusieurs CV depuis le catalogue. & Réalisé \\
...
\begin{itemize}
  \item la page du catalogue de CV ;
...
Le passage du document brut à l'objet métier exploitable peut être résumé par le tableau \ref{tab:document-transformation}.

\begin{table}[H]
  \centering
  \caption{\label{tab:document-transformation}Transformation documentaire du CV et de l'offre}
  \begin{tabularx}{\textwidth}{>{\bfseries}p{3cm}X X}
    ...
    Texte brut du CV & Extraction structurée des signaux métier : identité, compétences, expériences, formation, langues, résumé & Profil exploitable dans le catalogue \\
    ...
    Offre structurée & Validation, normalisation des critères et stockage métier & Objet comparable avec les profils du catalogue \\
    \bottomrule
  \end{tabularx}
\end{table}

Le tableau \ref{tab:document-transformation} montre que le CV n'est plus un simple fichier et que l'offre n'est plus un simple formulaire. Tous deux deviennent des objets métier progressivement transformés pour être réutilisables par les recruteurs, comparables par les services de matching et mobilisables par les couches RAG et agentiques.


\subsection{Vue fonctionnelle globale}
Les usages couverts par ce sprint se regroupent en trois blocs.
\begin{enumerate}
  \item \textbf{Gestion des CV} : importer un document, parser son contenu, extraire les informations principales, stocker le profil dans le catalogue, le consulter, le rechercher, le supprimer et l'exporter.
...
Un parcours représentatif illustre l'apport du module : un recruteur dépose un CV, le système extrait d'abord le texte et les métadonnées principales, puis le profil devient consultable dans le catalogue. Les traitements plus lourds, comme l'embedding et le découpage en chunks, enrichissent ensuite le document pour les usages de recherche sémantique et de RAG. Cette séparation entre disponibilité rapide et enrichissement progressif évite de confondre ingestion documentaire et calcul IA complet, comme le montre la figure \ref{fig:cv-upload-pipeline}.

\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/cv-upload-pipeline}
  \caption{\label{fig:cv-upload-pipeline}Séquence d'upload, d'extraction et d'indexation d'un CV}
\end{figure}
...
Même si le c\oe{}ur du pipeline candidat est traité dans le chapitre suivant, ce module prépare déjà le lien métier principal : le passage de \textit{CV dans le catalogue} à \textit{candidat dans une offre}. Cette transition transforme un actif documentaire en objet de pipeline.
...
La figure \ref{fig:job-assignment-sequence} illustre cette création d'offre et son lien avec le pipeline candidat.

\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/job-assignment-sequence}
  \caption{\label{fig:job-assignment-sequence}Création d'un poste et affectation d'un CV au pipeline candidat}
\end{figure}

\section{Réalisation}

\subsection{Page de catalogue des CV}
La page de catalogue des CV présente la liste des profils importés, leurs informations principales et des indicateurs de synthèse utiles au recruteur. Elle permet :
\begin{itemize}
...
La page présente aussi un niveau de synthèse : nombre total de CV, top compétences, distribution des langues et tendance d'upload sur les sept derniers jours. Le dialogue de consultation détaillée expose les faits observés, les données manquantes et les risques documentaires avant toute action IA.
Cette interface montre ainsi que le catalogue n'est pas une simple liste de fichiers. C'est une base de profils analysés, consultables et réexploitables, avec une lecture à la fois opérationnelle, analytique et déjà branchée sur l'assistance agentique. Les figures \ref{fig:cv-pool-page} et \ref{fig:cv-pool-details} illustrent respectivement la gestion du catalogue et la consultation détaillée d'un profil.

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/cvpool-ta.png}
  \caption{\label{fig:cv-pool-page}Page de gestion du catalogue de CV.}
\end{figure}

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/cvpool-details.png}
  \caption{\label{fig:cv-pool-details}Dialogue de consultation détaillée d'un CV.}
\end{figure}
...
\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/cvpool-upload-states.png}
  \caption{\label{fig:cv-upload-states}File d'import : succès d'un CV valide et rejet explicite d'un format non autorisé.}
\end{figure}

\subsection{Détection de doublons}
Le projet intègre un mécanisme de détection des doublons dans \sourcecode{duplicate-detection.ts}. Le service compare notamment l'email extrait, la similarité de nom et le numéro de téléphone. Cette fonctionnalité améliore la qualité documentaire du catalogue et, par conséquent, la qualité du matching et de la recherche RAG qui s'appuient sur ces données.
...
L'offre d'emploi n'est donc pas traitée comme un simple formulaire administratif. Elle devient un objet de décision qui articule besoin métier, contraintes de recrutement et future lecture algorithmique. C'est précisément cette structuration qui permet ensuite de comparer de manière cohérente un poste, un profil, un screening et un résultat de matching. Les figures \ref{fig:job-management} et \ref{fig:create-job} montrent cette gestion des offres côté interface.

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/job-management.png}
  \caption{\label{fig:job-management}Catalogue des offres d'emploi.}
\end{figure}

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/create-job.png}
  \caption{\label{fig:create-job}Zone de création ou d'édition d'une offre d'emploi.}
\end{figure}
...
Ce point montre que ce module n'est pas isolé. Le détail d'une offre devient le point de convergence entre la demande de recrutement, la base documentaire des profils, les parcours directs du produit et les futurs mécanismes de décision, comme le montre la figure \ref{fig:job-cv-matching}.

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/job-cv-matching.png}
  \caption{\label{fig:job-cv-matching}Page détail d'une offre avec candidats associés et résultats de matching.}
\end{figure}
