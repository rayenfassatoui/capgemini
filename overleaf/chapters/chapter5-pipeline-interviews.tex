...
Le tableau \ref{tab:pipeline-backlog} détaille les user stories qui couvrent cette circulation du candidat.

\subsection{Backlog du sprint}
\begin{table}[H]
  \centering
  \caption{\label{tab:pipeline-backlog}Backlog fonctionnel du module pipeline candidat et entretiens}
...
\begin{itemize}
  ...
  \item le service d'historique des transitions candidat ;
...
Chaque mouvement est maintenant doublé d'une trace dans \sourcecode{candidate\_stage\_history}. Cette table conserve l'étape précédente, la nouvelle étape, l'acteur, la raison, la source du changement et la date. Elle complète donc le statut courant du candidat par une mémoire chronologique, indispensable pour comprendre comment une décision a été atteinte.
\paragraph{Tableau de transitions du pipeline candidat.}
La logique d'état peut être synthétisée par le tableau \ref{tab:pipeline-transitions}.

\begin{table}[H]
  \centering
  \caption{\label{tab:pipeline-transitions}Transitions principales du pipeline candidat}
...
La figure \ref{fig:candidate-state-diagram} complète cette lecture sous forme de diagramme d'états.


\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/candidate-state-diagram}
  \caption{\label{fig:candidate-state-diagram}Diagramme d'états du pipeline candidat}
\end{figure}
...
La répartition des responsabilités peut être résumée par le tableau \ref{tab:pipeline-role-matrix}.

\begin{table}[H]
  \centering
  \caption{\label{tab:pipeline-role-matrix}Rôles et responsabilités dans le pipeline candidat}
...
La figure \ref{fig:interview-sequence} formalise cette séquence de planification, notification et reporting.

\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/interview-sequence}
  \caption{\label{fig:interview-sequence}Séquence de planification, notification et reporting d'un entretien}
\end{figure}
...
\subsection{Historique auditable des transitions}
Le service \sourcecode{updateCandidateStage()} ne met pas seulement à jour le champ \sourcecode{stage}. Il écrit aussi une ligne immuable dans \sourcecode{candidate\_stage\_history}. Lors de l'affectation initiale, \sourcecode{assignCvToJob()} enregistre une création de pipeline avec la source \sourcecode{assignment}. Les évolutions suivantes peuvent provenir d'une action manuelle, d'un traitement groupé, d'une affectation manager, d'une affectation RH, d'un rapport d'entretien ou d'une action agentique confirmée.

Cette conception évite une limite classique des pipelines de recrutement : connaître uniquement l'état courant sans savoir qui l'a modifié ni pourquoi. Les fiches Manager et RH affichent cette mémoire via \sourcecode{CandidateStageHistoryTimeline}. Les détails d'acteur restent visibles selon le rôle, ce qui permet de concilier transparence métier et restriction d'accès aux informations internes.
...
La fiche affiche également la timeline \sourcecode{Stage history}, qui permet au manager de replacer son évaluation dans la chronologie complète du dossier sans perdre la séparation des responsabilités.
Cette logique reflète bien le rôle du manager dans le produit : il ne se contente pas de visualiser un profil, il produit une décision structurée sur la base d'un dossier enrichi et explicitement qualifié. La figure \ref{fig:manager-candidate-review} illustre cette fiche côté Manager.

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/manager-candidate-review.png}
  \caption{\label{fig:manager-candidate-review}Fiche candidat côté Manager.}
\end{figure}
...
La fiche RH reprend la même logique d'historique afin de vérifier le chemin déjà parcouru par le candidat avant la décision finale. Cette lecture réduit le risque de décision isolée, car l'étape RH s'appuie sur les traces TA et Manager déjà enregistrées.
Cette étape montre bien que le RH joue à la fois un rôle décisionnel et un rôle de communication formelle avec le candidat, avec un niveau de justification plus explicite qu'une simple lecture de notes brutes. Les figures \ref{fig:hr-decision-page} et \ref{fig:interview-scheduling} montrent la fiche RH et l'interface de planification.

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/hr-decision-page.png}
  \caption{\label{fig:hr-decision-page}Fiche candidat côté RH.}
\end{figure}

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/interview-scheduling.png}
  \caption{\label{fig:interview-scheduling}Interface de planification d'entretien.}
\end{figure}
...
La figure \ref{fig:calendar-view} montre cette coordination transverse sous forme de calendrier.

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/calendar-ta.png}
  \caption{\label{fig:calendar-view}Vue calendrier des entretiens.}
\end{figure}
...
\begin{itemize}
  ...
  \item il transforme chaque transition en preuve chronologique consultable ;
