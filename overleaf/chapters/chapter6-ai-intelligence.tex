...
L'objectif est d'intégrer l'intelligence artificielle au c\oe{}ur du processus de recrutement afin d'assister les utilisateurs sans rompre la cohérence métier ni la sécurité des données. Plus précisément, ce sprint vise à :
\begin{itemize}
  \item accélérer le tri initial des profils ;
  \item fournir des scores et synthèses de screening ;
  \item rendre possible une recherche sémantique sur le catalogue ;
...
Le tableau \ref{tab:ai-backlog} détaille les capacités IA livrées dans ce sprint.
\subsection{Backlog du sprint}
\begin{table}[H]
  \centering
  \caption{\label{tab:ai-backlog}Backlog fonctionnel du module intelligence artificielle}
  \begin{tabularx}{\textwidth}{>{\bfseries}p{1.3cm}p{3.2cm}X>{\raggedright\arraybackslash}p{2cm}}
    ...
    30 & Agent conversationnel & En tant qu'utilisateur autorisé, je peux interroger la plateforme via un agent multi-outils, obtenir une réponse exploitable, visualiser des indicateurs utiles et consulter une trace par phases. & Réalisé \\
    ...
    30-ter & Cartes de réponse typées & En tant qu'utilisateur, je peux lire des cartes candidat, pipeline ou gouvernance construites à partir des sorties d'outils vérifiées. & Réalisé \\
    ...
    32-bis & Évaluation agentique & En tant qu'équipe projet, nous pouvons vérifier automatiquement les garde-fous de grounding, de confirmation, de restitution structurée et de trace outil. & Réalisé \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  \item les cartes de réponse typées candidat, pipeline et gouvernance ;
  \item le regroupement de la trace d'outils par phases d'exécution ;
  \item le rapport d'évaluation quantitative du pipeline RAG ;
  \item la suite d'évaluation agentique et UI du chat.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Intelligence conversationnelle} : agent multi-outils, raisonnement en plusieurs étapes, accès contrôlé aux services métier, réponses ancrées dans les résultats disponibles, cartes typées et trace d'exécution par phases.
\end{enumerate}

\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/ai-levels}
  \caption{\label{fig:ai-levels}Niveaux complémentaires de l'intelligence artificielle du projet}
\end{figure}
...
Cette combinaison évite de dépendre d'un seul mécanisme. Certaines requêtes bénéficient d'une lecture très structurée, d'autres d'une lecture sémantique plus souple, et les cas complexes profitent de la fusion de plusieurs signaux. La figure \ref{fig:ai-screening-autopilot} illustre cette chaîne IA de screening et de préparation d'entretien.

\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/ai-screening-autopilot}
  \caption{\label{fig:ai-screening-autopilot}Chaîne IA de screening et de génération de guide d'entretien}
\end{figure}
...
Sur le plan technique, cet agent s'appuie sur un registre de plus de cinquante outils répartis en catégories : catalogue de CV, jobs, candidats, matching, entretiens, fonctions IA, communication, dashboard et administration. Le registre centralise les définitions, la validation Zod, l'exécution et le filtrage RBAC par rôle.
Le projet ne laisse cependant pas l'agent répondre librement sur des personnes qu'il n'a pas effectivement résolues. Le mécanisme de grounding reconstruit l'ensemble de candidats autorisés à partir des résultats d'outils et valide les noms présents dans la réponse finale. Cette logique complète les garde-fous définis dans la couche d'orchestration : nombre maximal d'étapes, volume de sortie, timeout, seuil d'échec d'outils, séparation entre faits observés et recommandations, restitution finale orientée décision et transformation des résultats en cartes typées lorsque cela améliore la lecture.
Ces garde-fous répondent à une exigence forte du domaine RH : une réponse fluide mais non justifiée peut être plus dangereuse qu'une absence de réponse. Le système préfère donc encadrer l'agent, afficher sa \textit{evidence trace}, regrouper les outils par phases d'exécution et pointer ses limites plutôt que maximiser artificiellement sa liberté de génération.
Pour les outils qui modifient les données, le projet ajoute un garde-fou supplémentaire : l'action n'est pas exécutée immédiatement. Le service \sourcecode{pending-agent-actions.ts} crée une entrée \sourcecode{pending\_agent\_actions} avec les arguments sanitizés, un résumé métier, un statut \sourcecode{pending} et une expiration. L'interface affiche alors une carte de confirmation enrichie, avec niveau de risque, entités affectées, impact attendu, compte à rebours et arguments exacts ; l'utilisateur peut confirmer ou annuler. Après confirmation, l'outil est exécuté, puis l'action passe à \sourcecode{executed} ou \sourcecode{failed}. Ainsi, l'agent peut proposer une mutation, mais la responsabilité finale reste explicitement humaine.

\section{Vue dynamique}

\subsection{Orchestration agentique}
Le flux global du chat est le suivant : authentification, construction du contexte, filtrage RBAC des outils, boucle agentique, demande de confirmation si l'outil modifie les données, exécution d'outil, sanitization, grounding, construction des métadonnées de preuve, génération éventuelle de graphiques et de cartes typées, puis diffusion SSE de la réponse. Cette architecture donne à l'agent une vraie capacité d'action tout en gardant la main sur le périmètre autorisé, sur la validation des entrées et sur les mutations sensibles.
...
Pour bien situer les apports du sprint, il est utile de distinguer les principaux niveaux d'accès à l'information, comme le fait le tableau \ref{tab:search-mode-comparison}.

\begin{table}[H]
  \centering
  \caption{\label{tab:search-mode-comparison}Comparaison entre recherche simple, recherche sémantique, RAG et chat agentique}
...
\paragraph{Justification critique des choix IA.}
Le choix du RAG, du retrieval hybride et de l'agent outillé résulte d'un compromis entre pertinence, explicabilité, coût et gouvernance. Un modèle génératif seul serait plus simple à appeler, mais il ne pourrait pas garantir que la réponse s'appuie sur les CV, offres et historiques réellement présents dans la plateforme. À l'inverse, une recherche SQL classique serait rapide et déterministe, mais elle resterait insuffisante pour des formulations métiers proches sans mots-clés identiques. Le RAG hybride est donc retenu parce qu'il combine une réponse contextualisée avec des sources récupérées, tandis que l'agent outillé est retenu uniquement pour les demandes qui nécessitent plusieurs opérations métier contrôlées.

\begin{table}[H]
  \centering
  \scriptsize
  \caption{\label{tab:ai-choice-rationale}Justification analytique des choix IA}
  \begin{adjustbox}{max width=\linewidth,center}
  \begin{tabularx}{\textwidth}{>{\bfseries\raggedright\arraybackslash}p{2.9cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
    \toprule
    Choix & Pourquoi ce choix & Compromis observé & Mécanisme de maîtrise \\
    \midrule
    RAG plutôt que génération seule & Les travaux sur le RAG montrent l'intérêt d'ancrer la génération dans des documents récupérés \netref{web:rag-lewis}\netref{web:rag-survey-ieee}. & Latence plus élevée et dépendance à la qualité de l'index. & Chunks par section, citations, cache, fallback et évaluation quantitative. \\
    Hybrid Search plutôt que vectoriel seul & Les compétences exactes, certifications et technologies nommées exigent un signal lexical ; les synonymes et formulations proches exigent un signal dense. & Deux listes doivent être fusionnées et rerankées. & RRF, inspiré de la fusion de rangs \netref{web:rrf-cormack}, puis reranking et seuils. \\
    Réécriture de requête ciblée & Les requêtes longues de recruteurs mélangent souvent besoin, contexte et contraintes. & Appel LLM supplémentaire, donc coût et latence. & Activation seulement si la complexité le justifie, timeout et fallback lexical. \\
    Agent outillé plutôt que chatbot libre & Les approches ReAct et Toolformer montrent l'intérêt de coupler raisonnement et actions outillées \netref{web:react-yao}\netref{web:toolformer}. & Risque d'action non désirée ou de réponse hors périmètre. & Registre d'outils, RBAC, validation Zod, confirmations et grounding. \\
    Pas de fine-tuning initial & Les données CV et offres changent fréquemment ; les faits doivent rester actualisables sans réentraîner un modèle. & Moins de spécialisation linguistique qu'un modèle ajusté. & Prompts spécialisés, retrieval contextualisé et perspectives de fine-tuning ciblé. \\
    \bottomrule
  \end{tabularx}
  \end{adjustbox}
\end{table}

Le tableau \ref{tab:ai-choice-rationale} montre que la performance attendue n'est pas seulement une performance algorithmique. Elle dépend aussi de la capacité à expliquer, limiter et vérifier l'IA dans un domaine sensible. Le projet privilégie donc des mécanismes réversibles et auditables : récupérer avant de générer, fusionner plusieurs signaux plutôt qu'en absolutiser un seul, confirmer avant de modifier, et mesurer les gains au lieu de les affirmer.
La figure \ref{fig:rag-pipeline} représente ensuite le pipeline RAG hybride retenu dans le projet.


\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/rag-pipeline}
  \caption{\label{fig:rag-pipeline}Pipeline RAG hybride du projet}
\end{figure}
...
Cette intégration directe évite de créer une zone IA déconnectée du reste de l'application. Les suggestions et scores apparaissent au moment où le recruteur doit réellement arbitrer, comme l'illustre la figure \ref{fig:ai-score-view}.

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/ai-score-view.png}
  \caption{\label{fig:ai-score-view}Vue de score IA d'un candidat sur une offre.}
\end{figure}
...
L'IA n'est donc pas utilisée seulement pour retrouver ou classer, mais aussi pour préparer une interaction humaine plus pertinente avec le candidat. La figure \ref{fig:autopilot-guide} montre cette interface d'accompagnement.

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/autopilot-guide.png}
  \caption{\label{fig:autopilot-guide}Interface d'Auto-Pilot d'entretien.}
\end{figure}

\subsection{Page statistiques, workspace agentique et réponses sourcées}
La page statistique du recruteur charge les statistiques du catalogue, les statistiques des jobs et les \textit{smart insights}. Le composant \sourcecode{StatisticsChat} complète cette vue par une interaction conversationnelle SSE, mais il n'est plus seul : la navigation expose aussi une page \sourcecode{/agent} plein écran conçue pour les analyses longues, la consultation de l'historique et les traces détaillées.
Cette couche conversationnelle n'est pas décorative. Elle ouvre un accès agentique aux données du système, avec mémoire conversationnelle, outils filtrés par rôle, exécution traçable et prompts contextuels injectés depuis les pages métier. Les boutons \textit{Ask Agent} disséminés dans le catalogue, les offres, les fiches Manager, les fiches RH et les surfaces Admin réutilisent les données du contexte courant pour préremplir la demande.
Du point de vue métier, l'apport du sprint est concret. La recherche sémantique réduit l'effort de repérage des profils proches, le RAG améliore la justification des réponses complexes, et le chat agentique réduit le coût de navigation entre plusieurs écrans spécialisés. Le recruteur ne gagne donc pas seulement en rapidité ; il gagne aussi en lisibilité sur les raisons qui conduisent à une recommandation, un classement ou une synthèse. La figure \ref{fig:agent-workspace} montre cette restitution sourcée. Les figures \ref{fig:agent-pipeline-funnel} et \ref{fig:agent-skill-gap} isolent les deux cartes graphiques observées dans ce même workspace : la distribution des candidats par étape et l'écart entre demande des offres et compétences disponibles dans le vivier.
\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/agent-workspace-references.png}
  \caption{\label{fig:agent-workspace}Workspace agentique : demande analytique, preuves sourcées et amorce de restitution du pipeline.}
\end{figure}

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/agent-pipeline-funnel.png}
  \caption{\label{fig:agent-pipeline-funnel}Funnel de pipeline affiché dans le workspace agent.}
\end{figure}

\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/agent-skill-gap.png}
  \caption{\label{fig:agent-skill-gap}Écart demande--offre de compétences affiché dans le workspace agent.}
\end{figure}
\begin{figure}[H]
  \centering
  \appcapture{figures/screenshots/agent-confirmation-trace.png}
  \caption{\label{fig:agent-confirmation-trace}Confirmation humaine et trace d'exécution d'une action agentique sensible.}
\end{figure}
\subsection{Fonctionnement détaillé du chat analytique}
Le chat du projet n'est pas un simple champ de texte connecté à un LLM. Il constitue une interface de pilotage qui permet à l'utilisateur autorisé de poser des questions métier directement sur l'état réel de la plateforme. Un recruteur peut par exemple demander un résumé du catalogue, interroger la répartition des jobs, rechercher des candidats correspondant à un besoin précis ou demander une comparaison appuyée sur les données disponibles.
...
La dernière itération améliore aussi la lisibilité de cette couche. Les sorties d'outils ne restent pas uniquement dans un journal technique : elles peuvent être transformées en cartes de réponse typées. Une carte candidat expose par exemple le fit, les compétences, les écarts et un lien vers le CV ; une carte pipeline résume les compteurs et les goulets d'étranglement ; une carte gouvernance met en avant les lignes d'audit, les acteurs et les événements à risque. En parallèle, le composant de trace regroupe les outils en phases \textit{Planning}, \textit{Retrieval}, \textit{Analysis}, \textit{Confirmation}, \textit{Execution} et \textit{Verification}. Cette organisation rend le raisonnement agentique plus compréhensible pour l'utilisateur métier.
...
\paragraph{Mode de génération des figures IA.}
Les graphiques produits par l'assistant ne sont pas des images libres dessinées par le modèle. Le modèle peut contribuer à l'interprétation, mais la figure elle-même est générée par le backend à partir d'objets typés : source de données, série, libellés, valeurs numériques, type de graphique et liens de preuve. Cette séparation réduit le risque d'hallucination visuelle : l'IA ne dessine pas une courbe inventée, elle déclenche ou commente une restitution construite à partir des données validées du système. Dans le mémoire, les diagrammes d'architecture sont également générés comme figures vectorielles \LaTeX{}/TikZ, ce qui les rend reproductibles dans Overleaf et évite les problèmes de résolution liés à des captures bitmap.

\begin{table}[H]
  \centering
  \caption{\label{tab:chat-visual-restitution}Restitutions visuelles générées par le chat analytique}
  \begin{tabularx}{\textwidth}{>{\bfseries}p{3.2cm}p{3.6cm}X}
    \toprule
    Type de question & Source de données & Restitution attendue \\
    \midrule
    Tendance du catalogue & Statistiques CV & Courbe des uploads de CV sur la fenêtre récente. \\
    ...
    Réponse candidat & Matching, recherche ou comparaison & Carte typée avec score, compétences, écarts, lien CV et action de suivi. \\
    Lecture pipeline & Dashboard ou insights & Carte typée avec compteurs, phase dominante et recommandation d'action. \\
    Audit gouvernance & Logs et confirmations IA & Carte typée avec volume d'audit, événements à risque et lien vers les vues admin. \\
    Trace outillée & Événements SSE de tools & Timeline par phases : récupération, analyse, confirmation, exécution et vérification. \\
    \bottomrule
  \end{tabularx}
\end{table}

Le tableau \ref{tab:chat-visual-restitution} montre que cette évolution complète la logique de grounding : les graphiques et les cartes ne sont pas des illustrations décoratives générées librement, mais une autre forme de restitution des mêmes faits observés. Elle répond aussi à un besoin d'usage très concret : pour une question d'analytics, une courbe ou une barre comparative est souvent plus lisible qu'un paragraphe, tandis qu'une carte typée rend plus immédiatement visibles les métriques, les risques et les actions disponibles.
...
\begin{table}[H]
  \centering
  \caption{\label{tab:rag-eval-results}Résultats de l'évaluation RAG phase 2}
  \begin{tabular}{lccc}
    \toprule
  ...
  \end{tabular}
\end{table}
Les valeurs de ce tableau proviennent de l'artefact d'évaluation de phase 2 \sourcecode{reports/rag-eval/phase2-gate.md}.

\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/rag-eval-bars}
  \caption{\label{fig:rag-eval-bars}Visualisation des métriques RAG et du compromis de latence}
\end{figure}

Comme le montrent le tableau \ref{tab:rag-eval-results} et la figure \ref{fig:rag-eval-bars}, la phase 2 améliore les métriques de pertinence mais rend visible un coût temporel majeur. Cette représentation graphique évite de présenter l'IA comme un gain univoque : elle montre simultanément l'amélioration de qualité et le prix opérationnel de cette amélioration.
...
En complément de cette évaluation retrieval, le projet contient désormais une suite d'évaluation agentique déterministe. Elle vérifie les politiques de prompt anti-hallucination, le blocage des demandes créatives hors périmètre, le grounding des classements candidats, la confirmation obligatoire des mutations, la génération des cartes candidat/pipeline/gouvernance, le masquage des traces sensibles et le regroupement de la timeline d'outils. L'état de validation observé est détaillé dans les artefacts du projet : 125 tests passent sur la suite Vitest complète.

\subsection{Analyse de complexité, coût et pistes d'optimisation}
L'étude expérimentale doit être complétée par une lecture de complexité, car un pipeline IA utile en démonstration peut devenir difficile à exploiter si son coût croît trop rapidement avec le volume de CV, de chunks ou d'appels modèle. Dans ce projet, les principaux coûts proviennent de quatre sources : génération d'embeddings, recherche vectorielle, réécriture/reranking LLM et boucle agentique multi-outils.

\begin{table}[H]
  \centering
  \scriptsize
  \caption{\label{tab:complexity-analysis}Analyse de complexité et de coût du sous-système IA}
  \begin{adjustbox}{max width=\linewidth,center}
  \begin{tabularx}{\textwidth}{>{\bfseries\raggedright\arraybackslash}p{2.7cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
    \toprule
    Opération & Complexité dominante & Coût ou risque & Optimisation retenue ou proposée \\
    \midrule
    Embedding CV et chunks & Proportionnelle au nombre de textes indexés et à la dimension vectorielle $d=1024$. & Coût API et stockage d'environ $1024$ valeurs par embedding, soit environ 4 Ko hors métadonnées si valeurs flottantes 32 bits. & Traitement asynchrone après upload, batch embeddings, cache et réindexation ciblée. \\
    Recherche vectorielle & Dépend du nombre de chunks $N$, de la dimension $d$ et du type d'index. & Temps de recherche et mémoire augmentent avec le corpus. & pgvector, seuil de similarité, topK limité et invalidation par version d'index. \\
    Recherche lexicale & Dépend de la taille de l'index textuel et du nombre de résultats candidats. & Peut manquer les synonymes, mais reste rapide pour termes exacts. & Index \sourcecode{tsvector}, extraction de mots-clés et fusion avec le signal vectoriel. \\
    Fusion RRF et reranking & RRF est linéaire dans les listes récupérées ; le reranking dépend du nombre final de candidats. & Gain de pertinence au prix d'un traitement supplémentaire. & Limitation des listes vectorielle et lexicale, constante RRF et finalTopK maîtrisé. \\
    Réécriture de requête & Un appel LLM supplémentaire pour les requêtes complexes. & Latence et coût financier variables. & Déclenchement conditionnel, timeout, circuit breaker et fallback déterministe. \\
    Agent multi-outils & Dépend du nombre d'étapes, des appels d'outils et de la génération finale. & Risque de lenteur, d'échec d'outil ou de mutation excessive. & Limites d'étapes, confirmations, seuil d'échecs, streaming SSE et trace par phases. \\
    \bottomrule
  \end{tabularx}
  \end{adjustbox}
\end{table}

Le tableau \ref{tab:complexity-analysis} montre que l'optimisation prioritaire ne consiste pas à supprimer les étapes avancées, mais à mieux les déclencher. Les pistes les plus pertinentes sont donc : mettre en cache les embeddings et résultats de retrieval, réserver la réécriture aux requêtes ambiguës, diminuer le \textit{topK} quand la question est simple, réutiliser les résultats récents dans une même conversation, mesurer séparément latence vectorielle, latence lexicale, latence de reranking et latence de génération, puis envisager un fine-tuning léger uniquement sur des tâches stables comme la classification d'intention ou la normalisation des exigences.
...
\begin{itemize}
  ...
  \item il transforme les réponses analytiques et métier en graphiques ou cartes typées lorsque la visualisation apporte plus de valeur qu'un texte seul ;
  \item il rend la trace d'exécution plus lisible grâce à un regroupement par phases ;
  ...
  \item il ajoute une suite d'évaluation agentique qui vérifie les garde-fous et la restitution d'interface du chat.
\end{itemize}
...
Il faut surtout retenir que la valeur du module vient d'une intégration profonde : données enrichies, recherche hybride, outils métier, garde-fous, persistance des résultats, restitution visuelle ou typée des analyses, trace par phases et mesure de la performance.
