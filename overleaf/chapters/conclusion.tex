...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries}p{2.8cm}X X X}
    ...
    Chat analytique & Un utilisateur interroge le système depuis le chat flottant ou \sourcecode{/agent}. & Outils filtrés par rôle, streaming SSE, verdict lisible, preuves et graphiques si la demande est analytique. & Couvert par l'orchestration agentique, le grounding, les métadonnées d'évidence, les liens profonds et les cartes graphiques. \\
    \bottomrule
  \end{tabularx}
\end{table}

\paragraph{Validation technique automatisée.}
Sur le plan technique, la preuve automatisée principale a été exécutée via la commande \sourcecode{bun run test}. Cette exécution passe avec succès et valide 37 tests répartis sur 7 fichiers de test Vitest.
