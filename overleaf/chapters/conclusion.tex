...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries}p{2.8cm}X X X}
    ...
    Administration et gouvernance & Un administrateur consulte les vues système, analytics, activité, emails et onboarding. & Accès à une supervision consolidée, à des traces d'audit et à des exports exploitables. & Couvert par les vues administrateur, les logs, les notifications, les exports et les panneaux d'évidence source-backed du sprint 5. \\
    Chat analytique & Un utilisateur autorisé interroge le système en langage naturel depuis le chat flottant ou depuis le workspace \sourcecode{/agent}. & Filtrage des outils par rôle, réponse diffusée en SSE, séparation entre faits observés et recommandations, puis navigation vers les écrans source. & Couvert par le chat analytique, le registre d'outils, le grounding, les métadonnées d'évidence et les liens profonds de l'orchestration agentique. \\
    \bottomrule
  \end{tabularx}
\end{table}

\paragraph{Validation technique automatisée.}
Sur le plan technique, la preuve automatisée principale a été exécutée via la commande \sourcecode{bun run test}. Cette exécution passe avec succès et valide 32 tests répartis sur 5 fichiers de test Vitest.
