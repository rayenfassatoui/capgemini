...
La figure \ref{fig:global-use-case} synthétise les interactions majeures entre les quatre acteurs et les grands sous-systèmes métier.

\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/global-use-case}
  \caption{\label{fig:global-use-case}Cas d'utilisation global de la plateforme \projectname{}}
\end{figure}

\subsection{Processus métier cible}
Le diagramme de cas d'utilisation identifie les capacités du système, mais il ne suffit pas à montrer la circulation réelle d'une candidature entre les rôles. Pour répondre à cette limite, la figure \ref{fig:business-bpmn} formalise un processus métier de type BPMN simplifié : le TA initialise le besoin et le vivier, l'IA assiste le classement, le Manager évalue la compétence opérationnelle, puis HR arbitre la décision finale et déclenche l'onboarding ou la notification.

\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/recruitment-bpmn}
  \caption{\label{fig:business-bpmn}Processus métier cible du recrutement, sous forme BPMN simplifiée}
\end{figure}
...
Cette vue externe permet de distinguer clairement ce qui appartient au produit et ce qui dépend de services tiers. Le c\oe{}ur métier reste dans l'application et dans la base de données ; les services externes sont utilisés comme capacités spécialisées, sous contrôle des services applicatifs.
...
La figure \ref{fig:feature-architecture} présente cette organisation interne et ses principales frontières.

\begin{figure}[H]
  \centering
  \diagraminput{figures/diagrams/feature-architecture}
  \caption{\label{fig:feature-architecture}Architecture interne \textit{feature-driven} de la solution}
\end{figure}
...
La cohérence de l'architecture se comprend mieux lorsqu'on relie directement les besoins métier aux modules fonctionnels et aux choix techniques qui y répondent, comme le montre le tableau \ref{tab:business-architecture-correspondence}.
...
\begin{figure}[H]
  ...
  \caption{\label{fig:ai-orchestration}Orchestration de l'agent conversationnel et de ses outils}
\end{figure}
...
Le projet est développé dans un environnement JavaScript / TypeScript moderne. \sourcecode{Bun} est utilisé comme runtime principal, gestionnaire de dépendances et orchestrateur de scripts \netref{web:bun-docs}. \sourcecode{Next.js 16} fournit le socle full-stack de développement et d'exécution \netref{web:nextjs-docs}. L'environnement local repose sur \sourcecode{bun dev}, tandis que les scripts de base de données, de seed, de réindexation, de benchmark et d'évaluation utilisent également Bun.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries\raggedright\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{2.8cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
    ...
    Architecture feature-driven & Découpage par couches techniques ou microservices & Aligne chaque module sur une capacité métier : accès, vivier, jobs, pipeline, IA, gouvernance. & Évite un microservice prématuré et maintient une cohérence transactionnelle. \\
